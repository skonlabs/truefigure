"""Tests for the TrueFigure client (pure passthrough for events).

Asserts transport behavior (batching, retry, buffering, envelope, error mapping)
and that the event builders carry ZERO logic: no event_key, no canonicalization,
no enum/business validation. The server owns all of that.
"""
import os
import sys
import tempfile
...
# ---------- builders: pure shaping, no logic ----------
...
def test_builder_does_not_canonicalize_timestamp():
    ...
    assert e["payload"]["timestamp"] == "2026-07-15T15:02:11+01:00"  # verbatim; server canonicalizes


def test_builder_does_not_validate_enums_or_reject_fields():
    # An unusual action_type is NOT rejected client-side; extras are forwarded verbatim.
    e = events.activity("dep-1", user_ref="u1", timestamp="2026-07-01T00:00:00Z",
                        work_item_id="w1", action_type="anything_the_server_will_judge",
                        model_version_ref="gpt-x")
    assert e["payload"]["action_type"] == "anything_the_server_will_judge"
    assert e["payload"]["model_version_ref"] == "gpt-x"


def test_build_event_generic_with_scope():
...
def test_echo_uses_test_mode_header():
    echo_results = [{"index": 0, "status": "accepted", "event_key": "k0",
                     "echo": {"resolved_person": "p-77", "accepted": True, "reason": None}}]
...
def test_refused_figures_are_data_not_errors():
    fig = {"figures": [{"figure_id": "f1", "status": "refused", "margin_exceeds_effect": True}]}
