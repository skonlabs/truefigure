"""Event envelope builders — pure shaping, ZERO logic.

Nothing here validates, canonicalizes, keys, or dedups. The builders only
assemble the request envelope; the SERVER is the single source of truth for
validation, identity, deduplication, canonicalization, and measurement. Bad
input is not rejected locally — it is surfaced as a server error after send.

This module contains no proprietary algorithm of any kind (no event_key digest,
no timestamp canonicalization, no dedup-scope rules, no enum whitelists), so a
customer who downloads the SDK learns nothing beyond the published API contract.
Extra payload fields are forwarded verbatim via **extra.
"""
