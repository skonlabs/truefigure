-- =============================================================================
-- TrueFigure schema verification battery (schema v2.3, 31 tables)
-- Run after applying schema.sql: psql -d truefigure -f verification.sql
-- Every row must show PASS. Wire these into CI; they encode the invariants
-- the schema was certified against.
-- =============================================================================
SELECT 'A_enum_column_match_rule' AS check_name,
  CASE WHEN count(*)=0 THEN 'PASS' ELSE 'FAIL: '||string_agg(c.table_name||'.'||c.column_name,',') END AS result
...
SELECT 'G_fk_orphan_id_columns',
  CASE WHEN count(*)=0 THEN 'PASS'
       WHEN count(*)=1 AND min(c.table_name||'.'||c.column_name)='figures.method_id' THEN 'PASS (figures.method_id: documented deliberate TEXT registry reference)'
  ...
  AND NOT EXISTS (SELECT 1 FROM information_schema.key_column_usage k
                  JOIN information_schema.table_constraints tc ON tc.constraint_name=k.constraint_name AND tc.constraint_type='FOREIGN KEY'
